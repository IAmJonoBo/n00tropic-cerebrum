# Local Hooks & Proactive Guards

- **Pre-push hook** (`.git/hooks/pre-push`): runs `scripts/tidy-submodules.sh` (sync/update submodules, manifest gate, skeleton check) plus the manifest gate directly. Keeps gitlinks + manifest aligned before pushes.
- **Manifest gate (CI + local)**: `.dev/automation/scripts/manifest-gate.sh` fails when git repos or gitmodules are missing from `automation/workspace.manifest.json`.
