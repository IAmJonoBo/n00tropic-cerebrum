...
| Submodule has untracked changes you want to discard
| `+git -C <repo> checkout -- .+` or run `+workspace-health --repo-cmd <repo>:"git checkout -- ."+`

| Submodule is behind
| `git -C <repo> pull --ff-only`

| Tooling configs diverged
| `./.dev/automation/scripts/sync-trunk.py --pull` (automatically included in `--fix-all`)

| Only untracked files remain (root or submodule)
| `./.dev/automation/scripts/workspace-health.sh --clean-untracked`

| Workspace root dirty
| Commit or stash before release automation
|===
...
. Capture relevant ADRs in `1. Cerebrum Docs/ADR/`.
. Commit per-repo changes and push.
. Run `./.dev/automation/scripts/workspace-release.sh` when tagging a coordinated drop so release manifests stay in sync.
...
* Additional context and sample payload LIVE in xref:++1. Cerebrum Docs/AI_WORKSPACE_PLAYBOOK.adoc++[`AI_WORKSPACE_PLAYBOOK.md`].
